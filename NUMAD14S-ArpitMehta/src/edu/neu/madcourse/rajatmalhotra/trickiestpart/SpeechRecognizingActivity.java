...
public abstract class SpeechRecognizingActivity extends Activity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		boolean recognizerIntent = SpeechRecognitionUtil
				.isSpeechAvailable(this);
		if (!recognizerIntent) {
			speechNotAvailable();
		}
	 ...
	 * {@link #receiveWhatWasHeard(List, List)} when done might throw a
	 * {@link ActivityNotFoundException} if the device cannot respond to the
	 * Intent
	 */
	...
	/**
	 * @param code
	 *            If using {@link #recognizeDirectly(Intent) it will be the
	 *            error code from {@link SpeechRecognizer} if using
	 *            {@link #recognize(Intent)} it will be {@link #UNKNOWN_ERROR}.
	 */
